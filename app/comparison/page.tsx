  ...
  return (
    <AppShell status="ready">
      ...
      {hydrated && versions.length >= 2 && current && proposed && <>
        ...
        <section className="comparison-summary"><article className="version-summary"><span>BASELINE</span><strong>Version {current.version}</strong><p>{current.name || "Untitled decision canvas"}</p><small>{current.stages.length} stages</small></article><div className="comparison-arrow" aria-hidden="true">→</div><article className="version-summary proposed"><span>COMPARE TO</span><strong>Version {proposed.version}</strong><p>{proposed.name || "Untitled decision canvas"}</p><small>{proposed.stages.length} stages</small></article><div className="impact-summary"><div><strong className="good">{changedStages.filter((item) => item.change === "added").length}</strong><span>Added</span></div><div><strong className="bad">{changedStages.filter((item) => item.change === "removed").length}</strong><span>Removed</span></div><div><strong>{changedStages.filter((item) => item.change === "modified").length}</strong><span>Modified</span></div></div></section>
