...
function environmentLabel(value: CanvasVersion["environment"]) {
  return value === "production" ? "Production" : value === "staging" ? "Staging" : "Development";
}

function goLiveSummary(value: string) {
  if (!value) return "Go-live not set";
  const date = new Date(`${value}T00:00:00`);
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const days = Math.ceil((date.getTime() - today.getTime()) / 86_400_000);
  const formatted = new Intl.DateTimeFormat(undefined, { day: "2-digit", month: "short", year: "numeric" }).format(date);
  return days > 0 ? `${formatted} · ${days} days pending` : days === 0 ? `${formatted} · go-live today` : `${formatted} · ${Math.abs(days)} days past target`;
}
  ...
  return (
    <AppShell status="ready">
      ...
      {hydrated && versions.length >= 2 && current && proposed && <>
        <section className="local-compare-controls"><label><span>BASELINE ENVIRONMENT / VERSION</span><select value={currentId} onChange={(event) => setCurrentId(event.target.value)}>{versions.map((version) => <option key={version.id} value={version.id}>{environmentLabel(version.environment)} · Version {version.version} · {version.name || "Untitled"}</option>)}</select></label><span className="comparison-arrow">→</span><label><span>COMPARE ENVIRONMENT / VERSION</span><select value={proposedId} onChange={(event) => setProposedId(event.target.value)}>{versions.map((version) => <option key={version.id} value={version.id}>{environmentLabel(version.environment)} · Version {version.version} · {version.name || "Untitled"}</option>)}</select></label></section>
        <section className="comparison-summary"><article className="version-summary"><span>BASELINE · {environmentLabel(current.environment).toUpperCase()}</span><strong>Version {current.version}</strong><p>{current.name || "Untitled decision canvas"}</p><small>{current.stages.length} stages · {current.budget ? `${current.budgetCurrency || "USD"} ${current.budget}` : "No budget"} · SLA {current.sla ? `${current.sla} ${current.slaUnit || "days"}` : "not set"}</small><small>{goLiveSummary(current.goLiveDate)}</small></article><div className="comparison-arrow" aria-hidden="true">→</div><article className="version-summary proposed"><span>COMPARE TO · {environmentLabel(proposed.environment).toUpperCase()}</span><strong>Version {proposed.version}</strong><p>{proposed.name || "Untitled decision canvas"}</p><small>{proposed.stages.length} stages · {proposed.budget ? `${proposed.budgetCurrency || "USD"} ${proposed.budget}` : "No budget"} · SLA {proposed.sla ? `${proposed.sla} ${proposed.slaUnit || "days"}` : "not set"}</small><small>{goLiveSummary(proposed.goLiveDate)}</small></article><div className="impact-summary"><div><strong className="good">{changedStages.filter((item) => item.change === "added").length}</strong><span>Added</span></div><div><strong className="bad">{changedStages.filter((item) => item.change === "removed").length}</strong><span>Removed</span></div><div><strong>{changedStages.filter((item) => item.change === "modified").length}</strong><span>Modified</span></div></div></section>
